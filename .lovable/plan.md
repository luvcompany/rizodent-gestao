...
## 4. Alertas de segurança do banco

- Nenhuma tabela está aberta para visitantes não autenticados, e as 8 tabelas internas sem regra estão totalmente fechadas (correto).
- Três políticas de escrita (mapeamento de anúncios, destinatários de campanha, notificações) só checam "está logado" e dependem de uma segunda regra para o isolamento entre clínicas. Funciona hoje, mas basta alguém mexer na segunda regra para abrir vazamento entre clínicas. Proponho colocar a checagem da clínica também na própria política de escrita.
- Várias políticas estão aplicadas ao público em vez de apenas a usuários logados. Sem impacto atual, mas convém restringir.
- As funções internas do banco estão com caminho fixo e protegidas; as únicas sem essa proteção são de extensões (busca por texto e vetores), não do sistema.

## 5. Uploads sem limite de tamanho
...
## 6. Qualidade do código (melhorias, sem urgência)
...
## 7. Sinais para observar

O banco acumulou 6,7 milhões de transações revertidas desde o último reinício — número alto que sugere conflito recorrente em gatilhos ou tentativas repetidas de gravação. Vale investigar antes de virar lentidão. As falhas antigas da fila de automações (652 do erro "authHeader is not defined") já não ocorrem desde junho.
...
- RLS: políticas `Authenticated can insert/update ad_id_mapping`, `Staff can * crm_broadcast_recipients`, `Tenant members can insert notifications` — adicionar `tenant_id = current_tenant_id()` no `WITH CHECK` e `TO authenticated` nas políticas hoje em `public`.
...
1. Filtro de origem + normalização das variantes (itens 1) — só código de tela.
2. Paginação real das Conversas (item 2) — só código de tela.
3. Limpeza e retenção dos logs do banco (item 3) — migration + cron.
4. Endurecer as três políticas e limites de upload (itens 4 e 5) — migration.
5. Higiene de código e colunas explícitas (item 6) — incremental.
