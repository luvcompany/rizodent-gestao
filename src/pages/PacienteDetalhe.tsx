...
const PacienteDetalhe = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setClinicas(cls || []);
      if (pac) {
        setEditNome((pac as any).nome);
        setEditTelefone((pac as any).telefone);
        setEditCidade((pac as any).cidade || "");
        setEditEmail((pac as any).email || "");
        setEditOrigem((pac as any).origem || "");
        setEditNomeAnuncio((pac as any).nome_anuncio || "");
      }
      setLoading(false);
    };
    load();
  ...
  const grupos = new Map<string, { tratamento: any; pagamentos: any[]; total: number }>();
  pagamentos.forEach((p) => {
    const tid = p.tratamento_id || "sem_tratamento";
    if (!grupos.has(tid)) {
      const trat = tratamentos.find(t => t.id === tid);
      grupos.set(tid, { tratamento: trat, pagamentos: [], total: 0 });
    }
    const g = grupos.get(tid)!;
    g.pagamentos.push(p);
        ...
        <AlertDialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>Isso excluirá permanentemente o paciente <strong>{paciente.nome}</strong>, todos os seus tratamentos e pagamentos.</AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
      ...
      <Card className="gradient-card border-border shadow-card">
        ...
        <CardContent className="space-y-3">
          {pagamentos.length === 0 ? (
            ...
            Array.from(grupos.entries()).map(([tid, group]) => (
