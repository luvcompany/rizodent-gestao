...
export default function CrmCalendario() {
  ...
  // === EVENTS VIEW (Kommo-style columns) ===
  const eventsView = useMemo(() => {
    const now = new Date();
    const todayStart = startOfDay(now);
    const tomorrowStart = addDays(todayStart, 1);
    const nextWeekStart = addDays(todayStart, 2);
    ...
    const late = filtered.filter((t) => t.status !== "done" && isBefore(new Date(t.due_date), todayStart)).sort((a, b) => new Date(b.due_date).getTime() - new Date(a.due_date).getTime());
    const today = filtered.filter((t) => t.status !== "done" && isSameDay(new Date(t.due_date), todayStart)).sort((a, b) => new Date(a.due_date).getTime() - new Date(b.due_date).getTime());
    const tomorrow = filtered.filter((t) => t.status !== "done" && isSameDay(new Date(t.due_date), tomorrowStart)).sort((a, b) => new Date(a.due_date).getTime() - new Date(b.due_date).getTime());
  ...
  const renderTaskCard = (task: Task, compact = false) => {
    ...
    return (
      <div
        ...
        className={cn(
          "border rounded-md p-2.5 cursor-pointer hover:shadow-md transition-all",
          statusColor(st)
        )}
      >
        ...
        {compact && (
          <div className="text-[10px] text-muted-foreground truncate mt-0.5">
            {format(new Date(task.due_date), "HH:mm")} {typeLabel}
          </div>
        )}
      </div>
    );
  ...
  const renderEventsColumn = (title: string, tasks: Task[], color: string) => (
    <div className="flex-1 min-w-[220px] flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-2">
        {tasks.length === 0 && (
          <div className="text-center text-xs text-muted-foreground py-8">Nenhum evento</div>
        )}
    ...
    <div className="flex flex-col h-full -m-6 p-4" style={{ height: "calc(100vh - 4rem)" }}>
      ...
      <div className="flex items-center justify-between mb-3 flex-shrink-0 gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <span className="text-sm text-muted-foreground">{totalCount} eventos</span>
        </div>
      </div>

      {/* NAV BAR for month/week views */}
      ...
      {view === "events" && (
        <div className="flex-1 flex gap-px bg-border rounded-lg overflow-hidden min-h-0">
          {renderEventsColumn("Tarefas Concluídas", eventsView.done, "green")}
          {renderEventsColumn("Eventos Atrasados", eventsView.late, "red")}
          {renderEventsColumn("Eventos de Hoje", eventsView.today, "blue")}
          {renderEventsColumn("Eventos de Amanhã", eventsView.tomorrow, "orange")}
          {renderEventsColumn("Próxima Semana", eventsView.nextWeek, "gray")}
        </div>
      )}
          ...
          <table className="w-full text-sm">
            <thead className="bg-secondary/50 sticky top-0 z-10">
              <tr className="text-left">
                <th className="px-3 py-2 text-xs font-semibold text-muted-foreground uppercase">Data de Vencimento</th>
                ...
                <th className="px-3 py-2 text-xs font-semibold text-muted-foreground uppercase">Tipo de Evento</th>
                ...
                return (
                  <tr
                    ...
                    className={cn(
                      ...
                      st === "late" && "bg-destructive/5",
                      st === "done" && "bg-green-500/5"
                    )}
                  >
                    ...
                    <td className="px-3 py-2.5">
                      <button
                        onClick={(e) => { e.stopPropagation(); navigate(`/crm/conversa/${t.lead_id}`); }}
            ...
            {days.map((day) => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    "bg-card p-1 min-h-[90px] cursor-pointer hover:bg-secondary/30 transition-colors relative",
                    !inMonth && "opacity-30",
                ...
                >
                  ...
                  <div className="space-y-0.5">
                    {dayTs.slice(0, 3).map((t) => {
                      ...
                      return (
                        <div
                          ...
                          className={cn(
                            "text-[10px] px-1 py-0.5 rounded truncate cursor-pointer font-medium",
                            st === "done" && "bg-green-500 text-white",
                            st === "late" && "bg-destructive text-white",
                            st === "pending" && "bg-primary/15 text-foreground"
                          )}
                        >
                    ...
                    {dayTs.length > 3 && (
                      <div className="text-[9px] text-muted-foreground pl-1">+{dayTs.length - 3} Tarefas</div>
                    )}
                  </div>
                </div>
              );
      ...
      {view === "week" && (
        <>
          ...
          <div className="grid flex-shrink-0" style={{ gridTemplateColumns: "50px repeat(7, 1fr)" }}>
            ...
            {days.map((day) => (
              <div
                key={day.toISOString()}
                className={cn(
                  "text-center py-2 text-xs font-medium border-b border-border",
          ...
          {/* All-day tasks row */}
          <div className="grid flex-shrink-0 border-b border-border" style={{ gridTemplateColumns: "50px repeat(7, 1fr)" }}>
            <div className="text-[10px] text-muted-foreground p-1 border-r border-border">Todo dia</div>
            {days.map((day) => {
              const key = format(day, "yyyy-MM-dd");
              const allDay = (tasksByDay.get(key) || []).filter((t) => {
                const h = new Date(t.due_date).getHours();
                return h === 23 && new Date(t.due_date).getMinutes() === 59;
              });
              return (
                <div key={key} className="border-r border-border p-0.5 min-h-[28px]">
                  {allDay.slice(0, 3).map((t) => {
                    ...
                    return (
                      <div
                        ...
                        onClick={() => setSelectedTask(t)}
                        className={cn(
                          "text-[9px] px-1 py-0.5 rounded cursor-pointer truncate mb-0.5 font-medium",
                          st === "done" && "bg-green-500 text-white",
                          st === "late" && "bg-destructive text-white",
                          st === "pending" && "bg-primary/15 text-foreground"
                        )}
            ...
            <div className="grid" style={{ gridTemplateColumns: "50px repeat(7, 1fr)" }}>
              {hours.map((hour) => (
                <div key={hour} className="contents">
                  <div className="text-[10px] text-muted-foreground text-right pr-2 pt-1 border-r border-border h-[60px]">
                    {String(hour).padStart(2, "0")}:00
                  </div>
                  {days.map((day) => {
                    const key = format(day, "yyyy-MM-dd");
                    const hourTasks = (tasksByDay.get(key) || []).filter((t) => {
                      const d = new Date(t.due_date);
                      return d.getHours() === hour && !(d.getHours() === 23 && d.getMinutes() === 59);
                    });
                    return (
      ...
      {/* Day summary panel (month view) */}
      {view === "month" && selectedDay && dayTasks.length > 0 && (
          ...
          {selectedTask && (() => {
            ...
            return (
              <div className="space-y-3 text-sm">
                <div className="flex items-center gap-2">
                  <Badge className={cn(statusBg(st))}>
                    {st === "done" ? "Concluída" : st === "late" ? "Atrasada" : "Pendente"}
                  </Badge>
                ...
                {assignedProfile && (
                  <div>
                    ...
                    <span className="font-medium">{assignedProfile.nome}</span>
                  </div>
                )}
                {selectedTask.notes && <p className="text-muted-foreground bg-secondary/50 p-2 rounded">{selectedTask.notes}</p>}
                <div className="flex gap-2">
                  <Button size="sm" variant="outline" className="flex-1" onClick={() => { setSelectedTask(null); navigate(`/crm/conversa/${selectedTask.lead_id}`); }}>
                    Ir para conversa
                  </Button>
                  {st !== "done" && (
