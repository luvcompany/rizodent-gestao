...
export default function CloserMetricas() {
  ...
  useEffect(() => {
    let vivo = true;
    (async () => {
      const { data } = await (supabase as any).rpc("closer_dashboard_metrics");
      if (vivo && data) setM(data as Metricas);
    })();
    return () => { vivo = false; };
  }, []);
  ...
  const cartoes = [
    {
      rotulo: "Hoje",
      valor: brl(m?.faturamento_dia ?? 0),
      apoio: "faturamento do dia",
      Icone: Coins,
    ...
    {
      rotulo: "Total",
      valor: brl(m?.faturamento_total ?? 0),
    ...
    {
      rotulo: "Fechamentos",
      valor: String(m?.fechamentos_mes ?? 0),
