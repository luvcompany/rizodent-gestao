...
import { Sticker, X, Search, ImagePlus } from "lucide-react";
...
export default function StickerGalleryPopover({ disabled, hidden, disabledReason, onPick }: Props) {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent side="top" align="start" sideOffset={8} className="w-[360px] p-0 max-h-[420px] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-2">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-4 gap-2">
              {/* Create tile (always visible, only hidden by search when there is a query) */}
              {!query.trim() && (
                <button
                  type="button"
              ...
              {filtered.map((s) => (
                <div key={s.id} className="relative group aspect-square">
                  <button
                    ...
                    className="w-full h-full rounded-lg bg-muted/30 hover:bg-accent transition-colors overflow-hidden flex items-center justify-center"
