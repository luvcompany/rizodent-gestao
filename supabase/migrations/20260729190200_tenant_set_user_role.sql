-- A tela de Usuários gravava direto em user_roles, que não tem policy PERMISSIVA de
-- escrita — a troca de função era um no-op silencioso (pré-existente). RPC SECURITY
-- DEFINER com a MESMA allowlist do tenant-create-user (sem escalada de privilégio).
...
-- Lado leitura do mesmo problema: admins do tenant não liam os papéis dos colegas
-- (lista mostrava "Sem função"). SELECT permissivo escopado ao tenant; a restritiva
-- user_roles_same_tenant continua valendo por cima.
