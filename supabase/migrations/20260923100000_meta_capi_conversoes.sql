...
--  WHERE table_name = 'crm_leads' AND column_name IN ('ctwa_clid', 'ctwa_clid_at');            -- 2 linhas
-- SELECT tgname FROM pg_trigger WHERE tgname LIKE 'trg_zz%meta_capi%';                         -- 4 gatilhos
-- SELECT jobname, schedule, active FROM cron.job WHERE jobname LIKE 'meta-capi%';               -- 2 jobs
-- SELECT count(*) FROM public.meta_capi_events;                                                 -- 0 (nasce vazia)
-- SELECT tenant_id, enabled FROM public.meta_capi_config;                                       -- vazio até o dono configurar
-- =============================================================================
