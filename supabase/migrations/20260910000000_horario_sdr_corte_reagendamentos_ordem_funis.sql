...
-- SELECT name, position FROM crm_pipelines WHERE tenant_id = '00000000-0000-0000-0000-000000000010' ORDER BY position;
-- SELECT corte_tolerancia_min FROM crm_rodizio_config;   -- 60
-- SELECT * FROM public.rodizio_horario_dia('00000000-0000-0000-0000-000000000010', '<sdr>', current_date);

-- ================================================================ 5. etapas padrão em todo funil novo
-- Pedido do dono (09/09): funis por procedimento (Prótese, Implante, Zigomático,
-- Faceta, Protocolo, Aparelho) com EXATAMENTE as etapas do Funil Principal, e
-- qualquer funil novo já nasce assim. Os fluxos (agendar → Agendado, reagendar,
-- compareceu, não compareceu, ciclo da SDR) procuram a etapa pelo nome dentro
-- do funil do lead, então basta ter as etapas com os mesmos nomes e marcas
-- (is_won / is_lost / visivel_para_sdr). Automações de etapa NÃO são copiadas.
...
-- Regras de etapa em qualquer funil: (a) nome repetido no mesmo funil não
-- entra; (b) etapa inserida pelo front no primeiro minuto de vida de um funil
-- que já nasceu com as etapas padrão também não entra — as telas antigas
-- (Integrações, "Duplicar funil") inserem as suas etapas logo depois de criar
-- o funil, e elas viriam por cima das clonadas; (c) as etapas do
-- administrador nascem ocultas para a SDR.
