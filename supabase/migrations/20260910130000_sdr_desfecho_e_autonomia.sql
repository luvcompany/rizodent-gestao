...
-- ============================================================ 5. a autonomia da SDR saiu daqui
-- A seção que dava à SDR escrita em crm_pipelines, crm_stages e crm_automations
-- foi retirada desta migration e refeita em
-- 20260910160000_sdr_autonomia_nos_funis_dela.sql.
--
-- Motivo: a revisão adversarial mostrou que liberar escrita nos funis da CLÍNICA
-- abre três buracos que nenhuma trava de policy fecha bem — apagar etapa leva os
-- leads em cascata (crm_leads.stage_id ON DELETE CASCADE), automação de entrada
-- criada por ela dispara para o lead de qualquer colega que entre na etapa, e
-- etapa com nome parecido ('Contrato fechado') sequestra o ciclo, porque o front
-- casa etapa por pedaço do nome e pega a primeira por position.
--
-- O dono decidiu, com os três riscos na mão: a SDR cria funil próprio e faz tudo
-- dentro dele; nos funis da clínica ela não escreve. É o que a 160000 implementa.
