...
--
-- 1. A coluna nova entrou e a estrutura antiga ficou sem autor:
-- SELECT count(*) FILTER (WHERE created_by IS NULL) AS da_clinica,
...
--
-- 2. As nove policies entraram como PERMISSIVE e nenhuma antiga saiu (609 é o
--    número de antes, em public + storage):
-- SELECT count(*) FROM pg_policies WHERE policyname LIKE 'sdr_%crm_pipelines'
--    OR policyname LIKE 'sdr_%crm_stages' OR policyname LIKE 'sdr_%crm_automations';
-- SELECT count(*) FROM pg_policies WHERE schemaname IN ('public','storage');
--
-- 3. ENSAIO, emulando a Bia (tudo desfeito pelo RAISE):
-- DO $t$
-- DECLARE
--   v_bia uuid := '9c32408d-d852-4c55-9637-b30a59a16c13';
--   v_principal uuid := 'a1b2c3d4-0001-4000-8000-000000000001';
--   v_etapa uuid; v_funil uuid; v_novo uuid; n integer; rep text := E'\n';
-- BEGIN
--   SELECT id INTO v_etapa FROM public.crm_stages
--    WHERE pipeline_id = v_principal AND public.normaliza_nome_etapa(name) = 'agendado';
--   PERFORM set_config('request.jwt.claims',
--     json_build_object('sub', v_bia, 'role', 'authenticated')::text, true);
--   EXECUTE 'SET LOCAL ROLE authenticated';
--
--   BEGIN DELETE FROM public.crm_stages WHERE id = v_etapa;
--         GET DIAGNOSTICS n = ROW_COUNT;
--         rep := rep || 'apagar etapa da clinica: ' || n || ' linha(s) (esperado 0)' || E'\n';
--   EXCEPTION WHEN OTHERS THEN rep := rep || 'apagar etapa da clinica: recusado (' || SQLERRM || ')' || E'\n'; END;
--
--   BEGIN UPDATE public.crm_stages SET name = 'Agendado X' WHERE id = v_etapa;
--         GET DIAGNOSTICS n = ROW_COUNT;
--         rep := rep || 'renomear etapa da clinica: ' || n || ' (esperado 0)' || E'\n';
--   EXCEPTION WHEN OTHERS THEN rep := rep || 'renomear: recusado' || E'\n'; END;
--
--   BEGIN UPDATE public.crm_stages SET visivel_para_sdr = true
--          WHERE pipeline_id = v_principal AND public.normaliza_nome_etapa(name) = 'nao contratado';
--         GET DIAGNOSTICS n = ROW_COUNT;
--         rep := rep || 'tornar visivel Nao contratado: ' || n || ' (esperado 0)' || E'\n';
--   EXCEPTION WHEN OTHERS THEN rep := rep || 'tornar visivel: recusado' || E'\n'; END;
--
--   BEGIN INSERT INTO public.crm_automations (stage_id, trigger_type, action_type, action_config, is_active, tenant_id)
--         VALUES (v_etapa, 'on_enter', 'send_template', '{}'::jsonb, true,
--                 '00000000-0000-0000-0000-000000000010') RETURNING id INTO v_novo;
--         rep := rep || 'criar automacao em etapa da clinica: PASSOU (defeito)' || E'\n';
--   EXCEPTION WHEN OTHERS THEN rep := rep || 'criar automacao em etapa da clinica: recusado' || E'\n'; END;
--
--   BEGIN INSERT INTO public.crm_pipelines (tenant_id, name)
--         VALUES ('00000000-0000-0000-0000-000000000010', 'ENSAIO funil da Bia')
--         RETURNING id INTO v_funil;
--         rep := rep || 'criar funil: ok, id ' || v_funil::text || E'\n';
--         rep := rep || '  allowed_roles: '
--             || (SELECT COALESCE(array_to_string(allowed_roles, ','), '(nulo)')
--                   FROM public.crm_pipelines WHERE id = v_funil) || E'\n';
--         INSERT INTO public.crm_stages (pipeline_id, name, position, tenant_id, visivel_para_sdr)
--         VALUES (v_funil, 'ENSAIO etapa', 99, '00000000-0000-0000-0000-000000000010', true);
--         rep := rep || 'criar etapa no funil dela: ok' || E'\n';
--   EXCEPTION WHEN OTHERS THEN rep := rep || 'criar funil/etapa: FALHOU (' || SQLERRM || ')' || E'\n'; END;
--
--   EXECUTE 'RESET ROLE';
--   RAISE EXCEPTION 'ENSAIO AUTONOMIA (desfeito): %', rep;
-- END $t$;
-- =============================================================================
