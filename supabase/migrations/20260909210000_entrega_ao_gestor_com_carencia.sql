...
-- SELECT entrega_gestor_apos_min FROM crm_rodizio_config;                       -- 1440
-- SELECT jobname, schedule FROM cron.job WHERE jobname = 'sdr-entrega-ao-gestor';
-- SELECT count(*) FROM crm_entregas_gestor;                                     -- 0 no início
-- SELECT proname FROM pg_proc WHERE proname IN ('sdr_agenda_entrega_ao_gestor','sdr_entregas_pendentes','rodizio_definir_carencia_entrega');
-- Teste (como SDR, com um lead dela): marcar Compareceu → mensagem de sistema
-- "passa para o administrador em 1 dia (…)"; lead continua dela; linha em
-- crm_entregas_gestor; 24 h depois o cron entrega e anota 'comparecimento' no livro.
