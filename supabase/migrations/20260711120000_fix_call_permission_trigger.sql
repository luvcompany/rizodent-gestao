...
--   • A tabela whatsapp_call_permissions ficava com 0 linhas (nada era gravado).
...
--
-- Correções:
--   1) phone_number_id passa a ser NULLABLE (não temos como resolvê-lo de forma
--      confiável no trigger: whatsapp_numbers está vazia e a mensagem inbound não
--      traz whatsapp_number_id). A permissão é identificada por (tenant, telefone),
--      então phone_number_id é apenas metadado.
--   2) O INSERT do trigger passa a ser envolvido em BEGIN/EXCEPTION: gravar a
--      permissão é "best-effort" e JAMAIS pode quebrar o salvamento da mensagem.
-- ============================================================================
...
BEGIN
  ...
  BEGIN
    INSERT INTO public.whatsapp_call_permissions
      (tenant_id, whatsapp_number_id, consumer_phone, lead_id, status, approved_at, expires_at, raw_payload, updated_at)
    VALUES (
      NEW.tenant_id, NEW.whatsapp_number_id, v_phone, NEW.lead_id,
