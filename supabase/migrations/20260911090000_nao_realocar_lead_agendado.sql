-- =============================================================================
-- Lead agendado não é redistribuído: a realocação por silêncio pula as etapas
-- Agendado, Reagendado e Relacionamento.
--
-- PEDIDO DO DONO (10–11/09/2026): "quero que os funis agendado reagendado, e
-- relacionamento fechem automaticamente a conversa com o lead, evitando de uma
-- mensagem de confirmação como um 'Ok' de um lead, conte como conversa aberta e
-- esse lead seja redistribuído."
--
-- POR QUE ESTA MIGRATION EXISTE, se a de ontem (20260911070000) já fecha a
-- conversa nessas três etapas: porque FECHAR A CONVERSA NÃO RESOLVE O PEDIDO.
-- Duas coisas conferidas no banco de produção em 11/09/2026:
--
--   a) o "Ok" do paciente REABRE a conversa em segundos. Em public.messages há
--      DOIS gatilhos de reabertura: trg_zz_conversa_reabre_ao_enviar (equipe
--      manda mensagem humana) e trg_zz_conversa_reabre_ao_receber (o LEAD manda
--      qualquer mensagem). O segundo zera conversa_fechada_em no "Ok".
--   b) a realocação NÃO OLHA conversa fechada. Em
--      public.rodizio_realocar_sem_resposta a coluna conversa_fechada_em
--      aparece uma única vez, no WHERE do UPDATE final, como trava de
--      concorrência ("a linha mudou no meio?"), e não como filtro de candidato.
--
-- Logo, o caminho certo é por ETAPA: lead parado em Agendado, Reagendado ou
-- Relacionamento não entra na lista de candidatos da realocação por silêncio.
-- É direto, não depende de conversa fechada e nenhum "Ok" desfaz. A migration
-- de ontem continua valendo pelo que ela faz de fato (deixar a caixa limpa,
-- tirar a conversa da lista de abertas); esta aqui é a que protege a dona do
-- lead.
--
-- O QUE MUDA, EXATAMENTE: UM predicado a mais nos DOIS ramos do UNION ALL que
-- monta os candidatos (ligado e sombra) —
--   AND NOT (l0.stage_id = ANY (v_paradas))
-- — e o cálculo de v_paradas, uma vez por tenant, ao lado de v_funis e
-- v_etapas. O resto do corpo é cópia byte a byte do que está em produção hoje
-- (pg_get_functiondef em 11/09/2026, md5 do corpo ab3f3ef49cbcf9529532ca743f376bbb),
-- com o predicado de presença em_atendimento_por, o teto de ausência da dona e
-- a carência de abertura. Nada mais foi tocado.
--
-- DESEMPENHO (a pergunta certa): o predicado entra no WHERE do subselect, que é
-- avaliado ANTES do LIMIT 100 — o mesmo lugar onde rodizio_minutos_uteis já
-- custou caro e teve de sair para dentro do laço. Por isso NÃO se usa aqui um
-- NOT EXISTS correlacionado contra crm_stages normalizando o nome por
-- candidato. Monta-se o array de ids uma vez por tenant (a função já faz isso
-- com v_funis e v_etapas) e no WHERE sobra um teste de pertinência em memória
-- contra ~24 uuids, sem tocar em tabela nenhuma. Custo por rodada: uma varredura
-- de crm_stages por tenant (hoje 12 funis, algumas dezenas de linhas).
--
-- E A ETAPA "Reagendar"? NÃO ENTROU — o dono não a citou, e na minha leitura ela
-- não deve entrar. "Reagendar" é sala de espera: o paciente pediu para remarcar
-- e o novo horário AINDA NÃO EXISTE; é trabalho pendente da SDR, e é justamente
-- ali que o silêncio dela custa a consulta. As três etapas desta lista são o
-- contrário — o compromisso já está marcado (Agendado/Reagendado) ou o lead está
-- em nutrição sem hora marcada (Relacionamento). Some-se que "Reagendar" já tem
-- fim próprio: o cron jobid 34 (reagendar-expirado-2130utc, 18:30 America/Bahia)
-- esvazia a etapa todo dia, mandando para "Não compareceu" quem não deu horário
-- novo. Ou seja, ninguém apodrece lá dentro. Se o dono quiser incluí-la mesmo
-- assim, é uma palavra a mais na lista do SELECT abaixo — mas que seja decisão
-- dele, com esse efeito na mesa: a SDR poderia ficar com um pedido de remarcação
-- parado a tarde inteira sem que o motor o tirasse dela.
--
-- MEDIDA NO TENANT RIZODENT em 11/09/2026, para dar tamanho: entrada_todas_etapas
-- está LIGADA, então essas etapas hoje ESTÃO na lista de candidatos de verdade
-- (nem o filtro de "agendamento não cancelado" as protege, porque ele é o ramo
-- que entrada_todas_etapas desliga). Havia 16 leads em Agendado, 6 em
-- Relacionamento e 4 em Reagendado com dona, distribuídos e com inbound — 26
-- leads que podiam trocar de dona por um "Ok".
--
-- PERMISSÃO: mantida como está hoje no banco (proacl = só service_role, além do
-- dono). É função de cron; authenticated não a executa e não é esta migration
-- que vai abrir isso. Por isso o REVOKE inclui authenticated — é cópia do que a
-- migration 20260911030000 já deixou aplicado, não uma restrição nova.
-- =============================================================================
...
DECLARE
  ...
  v_paradas uuid[];   -- etapas em que o lead NÃO gira mais (agendado/reagendado/relacionamento)
...
BEGIN
  FOR c IN SELECT k.tenant_id FROM public.crm_rodizio_config k WHERE k.modo <> 'desligado' LOOP
    ...
    -- ETAPAS QUE NÃO GIRAM. Pedido do dono (11/09/2026): lead parado em
    -- Agendado, Reagendado ou Relacionamento não entra na fila da realocação por
    -- silêncio. A lista é montada UMA VEZ POR TENANT, aqui fora do laço dos
    -- leads, do mesmo jeito que v_funis e v_etapas logo acima — e por isso o
    -- filtro lá embaixo é só um teste de pertinência a um array de ~24 ids,
    -- avaliado em memória. A alternativa (NOT EXISTS correlacionado em
    -- crm_stages, normalizando o nome por candidato) rodaria dentro do WHERE do
    -- subselect, que é avaliado ANTES do LIMIT 100: é exatamente o defeito de
    -- desempenho que esta função já documenta no caso de rodizio_minutos_uteis.
    --
    -- NOME NORMALIZADO, NUNCA O NOME CRU (public.normaliza_nome_etapa): os nomes
    -- vêm do cliente e no banco existem com espaço no fim e acentuação/caixa
    -- variando, e cada um dos 9 funis do rodízio tem a sua cópia da etapa.
    --
    -- COALESCE PARA ARRAY VAZIO, DE PROPÓSITO: com v_paradas NULL o predicado
    -- "NOT (stage_id = ANY (NULL))" é NULL, o que reprovaria TODOS os candidatos
    -- e mataria a realocação em silêncio no cliente que não tem nenhuma dessas
    -- etapas cadastradas. Vazio reprova ninguém, que é o comportamento de antes.
    SELECT COALESCE(array_agg(s.id), ARRAY[]::uuid[]) INTO v_paradas
      FROM public.crm_stages s
     WHERE s.pipeline_id = ANY (v_funis)
       AND public.normaliza_nome_etapa(s.name) IN ('agendado', 'reagendado', 'relacionamento');
        ...
        FROM (
          ...
          SELECT l0.id, l0.assigned_to AS dona, l0.last_inbound_at, l0.distribuido_em AS desde, l0.conversa_fechada_em
           ...
           WHERE cfg.modo = 'ligado'
             ...
             -- ETAPA QUE NÃO GIRA: o lead já foi agendado, reagendado ou está
             -- em relacionamento. O trabalho de responder terminou, e o "Ok" de
             -- confirmação do paciente não é atendimento pendente — não pode
             -- custar o lead da dona. É por ETAPA e não por conversa fechada de
             -- propósito: qualquer mensagem do lead reabre a conversa
             -- (trg_zz_conversa_reabre_ao_receber), e conversa_fechada_em nem
             -- participa da escolha dos candidatos — ela só aparece na trava de
             -- concorrência do UPDATE final. Por etapa, nenhum "Ok" desfaz.
             AND NOT (l0.stage_id = ANY (v_paradas))
          ...
          SELECT l0.id, sb.para_user_id, l0.last_inbound_at, sb.criado_em, l0.conversa_fechada_em
           ...
           WHERE cfg.modo = 'sombra'
             ...
             AND l0.pipeline_id = ANY (v_funis)
             AND l0.stage_id = ANY (v_etapas)
             -- a mesma etapa que não gira do ramo ligado: o ensaio só vale se
             -- prevê o que o motor faria de verdade. Sem isto, o relatório de
             -- sombra prometeria realocações que, ligado, não acontecem.
             AND NOT (l0.stage_id = ANY (v_paradas))
...
COMMENT ON FUNCTION public.rodizio_realocar_sem_resposta() IS
  'Realoca por silêncio o lead cuja dona não respondeu dentro do limite, no relógio do expediente dela. NÃO considera candidato o lead parado em etapa Agendado, Reagendado ou Relacionamento (normaliza_nome_etapa) — por etapa, e não por conversa fechada, que qualquer mensagem do lead reabre.';

-- =============================================================================
-- VERIFICAÇÃO (só leitura, depois de aplicar)
--
-- SELECT
--   (SELECT pg_get_functiondef(p.oid) LIKE '%v_paradas%'
--      FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
--     WHERE n.nspname = 'public' AND p.proname = 'rodizio_realocar_sem_resposta') AS tem_filtro,
--   (SELECT count(*) FROM regexp_matches(
--      (SELECT pg_get_functiondef(p.oid) FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
--        WHERE n.nspname = 'public' AND p.proname = 'rodizio_realocar_sem_resposta'),
--      'NOT \(l0\.stage_id = ANY \(v_paradas\)\)', 'g')) AS ramos_filtrados,  -- esperado 2
--   (SELECT pg_get_functiondef(p.oid) LIKE '%em_atendimento_por%'
--      FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
--     WHERE n.nspname = 'public' AND p.proname = 'rodizio_realocar_sem_resposta') AS presenca_intacta;
--
-- Quem o filtro tira da fila, hoje, no tenant Rizodent:
--
-- WITH f AS (SELECT public.rodizio_funis('00000000-0000-0000-0000-000000000010') AS funis,
--                   public.rodizio_etapas_entrada('00000000-0000-0000-0000-000000000010') AS etapas),
--      p AS (SELECT COALESCE(array_agg(s.id), ARRAY[]::uuid[]) AS paradas
--              FROM public.crm_stages s, f
--             WHERE s.pipeline_id = ANY (f.funis)
--               AND public.normaliza_nome_etapa(s.name) IN ('agendado','reagendado','relacionamento'))
-- SELECT public.normaliza_nome_etapa(s.name) AS etapa, count(*) AS leads,
--        bool_or(l.stage_id = ANY (p.paradas)) AS protegido_agora
--   FROM public.crm_leads l JOIN public.crm_stages s ON s.id = l.stage_id, f, p
--  WHERE l.tenant_id = '00000000-0000-0000-0000-000000000010'
--    AND l.pipeline_id = ANY (f.funis) AND l.stage_id = ANY (f.etapas)
--    AND l.assigned_to IS NOT NULL AND l.distribuido_em IS NOT NULL
--    AND l.last_inbound_at IS NOT NULL AND NOT COALESCE(l.is_blocked, false)
--  GROUP BY 1 ORDER BY 2 DESC;
--
-- =============================================================================
-- ENSAIO (desfeito): dois leads de mentira, um em "Agendado" e um em
-- "Conversando", ambos calados muito além do limite e com a mesma dona. Roda a
-- função de verdade e confere que o de Agendado NÃO trocou de dona e o de
-- Conversando trocou. O RAISE EXCEPTION do fim desfaz tudo — os leads, as
-- atribuições, as mensagens de sistema, as notificações e o ponteiro do rodízio;
-- o pg_net do painel de TV também é enfileirado na transação e o rollback o
-- cancela. Rodar fora do horário de pico: a função pega advisory lock do tenant
-- e toca em leads reais dentro da transação que será desfeita.
--
-- DO $t$
-- DECLARE
--   v_tenant uuid := '00000000-0000-0000-0000-000000000010';
--   v_pipe uuid; v_ag uuid; v_conv uuid; v_dona uuid; v_colega uuid;
--   v_lead_ag uuid; v_lead_conv uuid; v_dona_ag uuid; v_dona_conv uuid; v_n integer;
-- BEGIN
--   SELECT p.id INTO v_pipe FROM public.crm_pipelines p
--    WHERE p.tenant_id = v_tenant AND p.name = 'Funil Principal';
--   SELECT s.id INTO v_ag   FROM public.crm_stages s
--    WHERE s.pipeline_id = v_pipe AND public.normaliza_nome_etapa(s.name) = 'agendado';
--   SELECT s.id INTO v_conv FROM public.crm_stages s
--    WHERE s.pipeline_id = v_pipe AND public.normaliza_nome_etapa(s.name) = 'conversando';
--   -- duas SDRs com o expediente ABERTO agora, senão não há para quem realocar
--   SELECT p.user_id INTO v_dona  FROM public.rodizio_pool(v_tenant) p WHERE p.aberta ORDER BY p.ordem LIMIT 1;
--   SELECT p.user_id INTO v_colega FROM public.rodizio_pool(v_tenant) p WHERE p.aberta AND p.user_id <> v_dona ORDER BY p.ordem LIMIT 1;
--   IF v_dona IS NULL OR v_colega IS NULL THEN RAISE EXCEPTION 'ensaio precisa de 2 SDRs com expediente aberto'; END IF;
--
--   INSERT INTO public.crm_leads (tenant_id, pipeline_id, stage_id, name, phone, assigned_to,
--                                 distribuido_em, last_inbound_at)
--   VALUES (v_tenant, v_pipe, v_ag,   'ENSAIO agendado',   '5577000000001', v_dona,
--           now() - interval '2 days', now() - interval '2 days')
--   RETURNING id INTO v_lead_ag;
--   INSERT INTO public.crm_leads (tenant_id, pipeline_id, stage_id, name, phone, assigned_to,
--                                 distribuido_em, last_inbound_at)
--   VALUES (v_tenant, v_pipe, v_conv, 'ENSAIO conversando', '5577000000002', v_dona,
--           now() - interval '2 days', now() - interval '2 days')
--   RETURNING id INTO v_lead_conv;
--
--   v_n := public.rodizio_realocar_sem_resposta();
--   SELECT assigned_to INTO v_dona_ag   FROM public.crm_leads WHERE id = v_lead_ag;
--   SELECT assigned_to INTO v_dona_conv FROM public.crm_leads WHERE id = v_lead_conv;
--   RAISE NOTICE 'realocou % lead(s); agendado ficou com a dona? % ; conversando trocou? %',
--     v_n, (v_dona_ag = v_dona), (v_dona_conv <> v_dona);
--   -- esperado: agendado ficou com a dona = t ; conversando trocou = t
--   RAISE EXCEPTION 'ensaio desfeito de propósito';
-- END $t$;
-- =============================================================================
