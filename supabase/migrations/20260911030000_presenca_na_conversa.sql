...
BEGIN
  ...
  END LOOP;
  RETURN v_n;
END $fn$;REVOKE ALL ON FUNCTION public.rodizio_realocar_sem_resposta() FROM PUBLIC, anon, authenticated;
...
--     AND table_name='crm_leads' AND column_name IN ('em_atendimento_por','em_atendimento_em')) AS colunas,
--   (SELECT presenca_segura_min FROM public.crm_rodizio_config
--     WHERE tenant_id='00000000-0000-0000-0000-000000000010') AS minutos,
--   (SELECT pg_get_functiondef(p.oid) LIKE '%em_atendimento_por%' FROM pg_proc p
--     JOIN pg_namespace n ON n.oid=p.pronamespace
--    WHERE n.nspname='public' AND p.proname='rodizio_realocar_sem_resposta') AS realocacao_respeita;
--
-- ENSAIO (desfeito): lead com presença de 1 minuto NÃO é realocado; com 20, é.
-- Rodar cada bloco separadamente, trocando o "now() - interval" do carimbo.
